...
export const TopUpRequestModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onSubmit,
  isSubmitting,
    ...
    const init = async () => {
      ...
      try {
        ...
        const {
          ...
          configured,
          walletTopUpEnabled,
          paystackEnabled: paystackAllowed,
        } = paystackResp;
        localPaystackEnabled = Boolean(
          publicKey && configured && walletTopUpEnabled && paystackAllowed,
        );
  ...
  const collectionFeePreview = useMemo(() => {
    ...
    if (!feeSettings || !isAmountValid || Number.isNaN(parsedAmount))
      return null;
    const paystackPercent =
    ...
    const platformPercent =
      ...
      feeSettings.platformFeePercent ??
      0;
    const delegateFees =
    ...
    if (delegateFees) {
      ...
      const agentPays =
        Math.round((parsedAmount / (1 - totalFeePercent / 100)) * 100) / 100;
      ...
      return {
        feePercent: totalFeePercent,
        feeAmount:
          Math.round(parsedAmount * (totalFeePercent / 100) * 100) / 100,
  ...
  const validateAmount = (): boolean => {
    ...
    if (Number.isNaN(parsedAmount))
      return (setError("amount", "Enter a valid number"), false);
    ...
    if (parsedAmount > 10_000)
      return (setError("amount", "Maximum amount is GH₵10,000"), false);
    return true;
  };
  ...
  const handlePaystackCheckout = async () => {
    ...
    try {
      ...
      if (/authentication failed|secret key/i.test(message)) {
        addToast(
          "Paystack is not correctly configured. Contact your administrator.",
          "error",
          8000,
        );
      } else if (/currency not supported/i.test(message)) {
        addToast(
          "Paystack does not support GHS on this account. Contact your administrator.",
          "error",
          10000,
        );
      } else {
  ...
  const handleManualSubmit = async () => {
    const description =
      form.description.trim() || "Wallet top-up request via WhatsApp";
    handleClose();
    await onSubmit(parsedAmount, description);
    addToast(
      `Top-up request of GH₵${parsedAmount} submitted. You'll be notified when it's processed.`,
      "success",
      5000,
    );
    openWhatsApp();
  ...
  return (
    <Dialog isOpen={isOpen} onClose={handleClose} size="md">
      {/* ── Header ── */}
      <DialogHeader className="border-b border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-text)]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <FaMoneyBillWave className="h-5 w-5 text-[var(--color-primary-700)]" />
          ...
          <Button
            ...
            aria-label="Close"
            onClick={handleClose}
            className="text-[var(--color-muted-text)] hover:text-[var(--color-text)]"
          >
            <svg
              className="w-5 h-5"
              fill="none"
        ...
        {!isBlocked && (
          <StepProgress
            ...
            labelClassName="text-[var(--color-secondary-text)]"
          />
        )}
      </DialogHeader>
      ...
      <DialogBody className="space-y-4">
        {/* Loading state */}
        {checkingPending && (
          <div className="flex items-center justify-center py-8 gap-3 text-[var(--color-muted-text)]">
        ...
        {!checkingPending && hasPendingRequest && (
          <Alert status="warning" variant="solid">
            You already have a pending top-up request. Please wait for it to be
            processed before making a new request.
          </Alert>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              >
                ...
                >
                  <FaWhatsapp
                    className={`h-4 w-4 ${mode === "request" ? "text-[var(--color-primary-700)]" : "text-[var(--color-muted-text)]"}`}
                  />
                </div>
                <div className="text-center leading-tight">
                  <p className="font-semibold">Admin Request</p>
                  <p
                    className={`mt-0.5 text-xs ${mode === "request" ? "text-[var(--color-primary-700)]" : "text-[var(--color-muted-text)]"}`}
                  >
                    Via WhatsApp
                  </p>
                </div>
            ...
            {mode === "instant" && collectionFeePreview && (
              <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-3 text-sm space-y-1.5">
                <p className="font-medium text-[var(--color-primary-700)] text-xs uppercase tracking-wide">
                  Fee breakdown
                </p>
                <div className="flex justify-between text-[var(--color-secondary-text)]">
                  ...
                  <span className="font-medium text-[var(--color-primary-700)]">
                    GH₵{collectionFeePreview.netCredit.toFixed(2)}
                  </span>
                </div>
                {collectionFeePreview.agentBearsFee ? (
                  <>
                    <div className="flex justify-between text-[var(--color-primary-700)]">
                      <span>
                        Processing fee (
                        {collectionFeePreview.feePercent.toFixed(2)}%)
                      </span>
                      <span>
                        + GH₵{collectionFeePreview.feeAmount.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between font-semibold border-t border-[var(--color-border)] pt-1.5">
                      ...
                      <span className="text-[var(--color-text)]">
                        GH₵{collectionFeePreview.agentPays.toFixed(2)}
                      </span>
                    </div>
                  </>
                ) : (
                  <p className="text-xs text-[var(--color-muted-text)] border-t border-[var(--color-border)] pt-1.5">
                    Processing fee is covered by the platform — you are charged
                    exactly GH₵{parsedAmount.toFixed(2)}.
                  </p>
                )}
              </div>
            )}
            ...
            {mode === "request" ? (
              <InfoBox>
                ...
                notified and can follow up via WhatsApp once submitted.
              </InfoBox>
            ) : (
              <InfoBox>
                {selectedPaymentMethod === "paystack" ? (
        ...
        {!isBlocked && step === 2 && (
          <div className="space-y-4">
            <h4 className="font-semibold text-[var(--color-text)]">
              Confirm Your {mode === "instant" ? "Payment" : "Request"}
            </h4>

            <div className="bg-[var(--color-surface)] rounded-xl p-4 space-y-3 border border-[var(--color-border)]">
              <SummaryRow
                label="Amount"
                value={`GH₵${parsedAmount.toFixed(2)}`}
              />
              {form.description && (
            ...
            {mode === "instant" ? (
              ...
              <InfoBox>
                After submitting, a WhatsApp message will open so you can notify
                the admin directly.
              </InfoBox>
            )}
          </div>
