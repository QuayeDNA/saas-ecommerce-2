...
export const TutorialPlayer: React.FC = () => {
  const { activeTutorial, activeStep, nextStep, prevStep, closeTutorial } =
    useTutorial();
  ...
  return createPortal(
    ...
    >
      ...
      {targetRect && (
        <div
          className="absolute rounded-lg border-4 border-[var(--color-primary-400)] pointer-events-none"
          style={{
      ...
      <div
        ref={tooltipRef}
        className="absolute bg-[var(--color-surface)] rounded-xl shadow-2xl border border-[var(--color-border)] w-[340px] max-w-[calc(100vw-32px)] pointer-events-auto text-[var(--color-text)]"
        style={{
          top: tooltipPos.top,
      ...
      >
        ...
        <div className="flex items-start justify-between gap-2 px-4 pt-4 pb-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="shrink-0 w-8 h-8 rounded-full bg-[var(--color-primary-50)] flex items-center justify-center">
              {step.icon ||
                (step.isTask ? (
          ...
          <button
            ...
            className="text-[var(--color-muted-text)] hover:text-[var(--color-text)] p-1 -m-1 shrink-0 transition"
              ...
              <div key={i} className="flex items-center">
                {s.isTask ? (
                  i < activeStep ? (
                    ...
                    <Circle className="w-3.5 h-3.5 text-[var(--color-primary-500)] fill-[var(--color-primary-100)]" />
                  ) : (
                    <Circle className="w-3.5 h-3.5 text-[var(--color-gray-300)]" />
                  )
                ) : (
                  <div
        ...
        {/* Progress bar */}
        <div className="mx-4 h-1 bg-[var(--color-control-bg)] rounded-full overflow-hidden">
          <div
